...
import vdomImg from "../../../../public/images/react/vdom-diffing.png";
import { Image } from "astro:assets";

<Image
  src={vdomImg}
  alt="Two VDOM trees side by side labeled Previous VDOM and Next VDOM. The trees are identical except one leaf node in the new tree has a highlighted border labeled changed. An arrow between them labeled diff points to a box below showing only that node with the label DOM update: one node."
