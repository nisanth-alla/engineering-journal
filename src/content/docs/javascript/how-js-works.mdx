...
When you call `fetch()` or `setTimeout()`, JavaScript doesn't sit there waiting. It hands the work to the browser's Web APIs (or Node's libuv for server-side). The browser manages the network request or the timer on a separate thread. When the work is done, the browser puts the callback into a queue. The event loop picks it up when the call stack is empty.
...
Macrotasks are lower-priority callbacks. `setTimeout`, `setInterval`, I/O events (click handlers, keyboard events), `requestAnimationFrame` go here.
