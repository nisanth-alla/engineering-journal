...
This works brilliantly for I/O-bound work. A Node server handling 10,000 concurrent HTTP requests doesn't need 10,000 threads. It needs one thread and a fast event loop. Each request spends most of its time waiting on database queries or external APIs, and Node fills those waiting gaps with other requests.
...
In the [worker pool experiment](https://github.com/nisanth-alla/distributed-systems-lab/tree/main/experiments/go/worker-pool), I process 100 simulated files. Each one takes 50 to 300 milliseconds.
...
Goroutines don't have that ceiling. The trade-off is that you have to think about shared state and synchronization, which Node mostly lets you ignore. That trade-off is worth it for the kind of systems work I'm doing in the lab.
