...
const phases: Phase[] = [
  {
    ...
    detail: "Timers don't guarantee exact execution time. A setTimeout(cb, 100) means 'run cb no sooner than 100ms.' If the event loop is busy with I/O callbacks, the timer fires late. This is why setTimeout(..., 0) doesn't mean 'immediately' — it means 'as soon as the current phase and all microtasks finish.'",
  },
  {
    ...
    detail: "Most I/O callbacks run in the Poll phase. This phase handles the edge cases: callbacks for certain system operations (like TCP errors) that were deferred to the next loop iteration. You rarely interact with this phase directly.",
  },
  {
    name: "Idle, prepare",
    color: "#6366f1",
    ...
    detail: "This phase is for Node's internal use. libuv does housekeeping here. You can't schedule work into this phase from JavaScript. It exists in the architecture but you never think about it.",
  },
  {
    name: "Poll",
    color: "#22c55e",
    what: "Retrieves new I/O events. Runs I/O callbacks. Blocks here when idle.",
    examples: ["fs.readFile callback", "http.get response", "database query result", "net.Socket data"],
  ...
  },
  {
    ...
    detail: "When a socket or handle is closed abruptly (socket.destroy()), the close callback fires here. If a socket closes normally via .end(), the close event is emitted elsewhere. This phase cleans up resources at the end of the loop.",
  },
];
  ...
  return (
    <div className="interactive-demo">
      ...
      <p style={{ fontSize: "0.85rem", color: "var(--sl-color-gray-2)", margin: "0 0 1rem" }}>
        Click a phase to see what runs there, or hit Animate to watch the loop cycle through all phases.
      </p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem", marginTop: "1rem" }}>
        {phases.map((phase, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => { if (!animating) setActivePhase(isActive ? null : i); }}
              style={{
                display: "flex",
                alignItems: "center",
                gap: "0.75rem",
      ...
      {activePhase !== null && (
        <div className="demo-output" style={{ marginTop: "0.75rem" }}>
          <div style={{ marginBottom: "0.5rem" }}>
            <strong style={{ color: phases[activePhase].color }}>
          ...
          <div style={{ marginBottom: "0.5rem" }}>
            ...
            {phases[activePhase].examples.map((ex, i) => (
              <code key={i} style={{ marginRight: "0.5rem" }}>{ex}</code>
            ))}
          </div>
      ...
      <div style={{
        ...
        <strong>Between every phase:</strong> Node drains the microtask queue (Promise callbacks, process.nextTick). 
        process.nextTick runs before Promise callbacks. This is why nextTick can starve I/O if used recursively.
      </div>
    </div>
  );
}
